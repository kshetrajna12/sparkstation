#!/usr/bin/env bash
# SGLang+DSpark tuned for INDEXING (full worker node, batch-friendly).
set -euo pipefail
...
KEY=$(cat "$CFG/api-key")
exec docker run --rm --name qwen38-sglang-idx --gpus all \
  --memory 120g --memory-swap 120g --shm-size 16g --network host --ipc=host \
  ...
  python3 -m sglang.launch_server \
    ...
    --max-mamba-cache-size 256 --max-running-requests 8 \
